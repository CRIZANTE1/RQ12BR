...
def calcular_nova_escada(calculadora):
    ...
    if 'calculo_realizado' in st.session_state and st.session_state.calculo_realizado:
        if st.button("Salvar Projeto no Histórico"):
            ...
            if 'figura_escada' in st.session_state:
                ...
                grafico_url = salvar_arquivo(grafico_bytes, f"grafico_{calculo_id}.png")
            
            # 2. Criar o dicionário do cálculo/projeto
            calculo = {
                'id': calculo_id,
                ...
                'foto_url': None, # Projetos não têm foto da escada
                'tipo': 'Projeto'
            }
            
            # 3. Salvar os dados no banco de dados PostgreSQL
            salvar_avaliacao(calculo)
